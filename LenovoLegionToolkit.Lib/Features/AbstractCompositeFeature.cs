    ...
    where T3 : IFeature<T>
{
    ...
    protected AbstractCompositeFeature(T1 feature1, T2 feature2, T3 feature3)
    {
        _feature1 = feature1 ?? throw new ArgumentNullException(nameof(feature1));
        _feature2 = feature2 ?? throw new ArgumentNullException(nameof(feature2));
        _feature3 = feature3 ?? throw new ArgumentNullException(nameof(feature3));
    ...
    protected virtual async Task<IFeature<T>?> GetFeatureLazyAsync()
    {
        ...
        if (await _feature3.IsSupportedAsync().ConfigureAwait(false))
            return _feature3;
